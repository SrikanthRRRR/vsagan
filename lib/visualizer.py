...
class Visualizer():
    ...
    def save_current_images(self, epoch, reals, fakes, fixed, identifier=None):
        ...
        if identifier is None:
            identifier = self.img_dir+ '/'
        else:
            identifier = self.img_dir + '/' + identifier + '_'

        #print(epoch)
        vutils.save_image(reals, '%sreals.png' %(identifier), normalize=True)
        vutils.save_image(fakes, '%sfakes.png' %(identifier), normalize=True)
        vutils.save_image(fixed, '%sfixed_fakes_%03d.png' %(identifier, epoch+1), normalize=True)
